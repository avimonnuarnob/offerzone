...
export interface ContextValue extends State {
  ...
  closeShop: () => void;
  toggleSidebar: () => void;
  closeSidebarIfPresent: () => false | void;
  openCart: () => void;
  closeCart: () => void;
  toggleCart: () => void;
  ...
  setUserAvatar: (_value: string) => void;
...
export const UIContext = React.createContext<State | undefined>(initialState);
...
export const useUI = () => {
  const context = React.useContext(UIContext) as ContextValue;
...
export function ManagedUIContext({
  children,
